...
internal class Agent : IAgent
{
    ...
    public void Start(CancellationToken cancellationToken)
    {
        ...
        _consumer.Subscribe(_settings.StopProcessTopic, _ => _processService.StopProcess(), cancellationToken);

        _processService.OnRestartProcess += (_, processModel) =>
        {
            _processService.StartProcess(processModel,
                outputData => _producer.Produce(_settings.StreamLogsTopic, outputData));
        };
    }
